...
def eval_gpt_open_ended(model, dataset, args, print_vis_token_meaning=True):
    ...
    with tqdm(total=len(dataset)) as epoch_pbar:
        ...
        for item in range(len(dataset)):
            prefix,  kg, tokens, mask, q_len = dataset[item]
            ...
            with autocast(dtype=torch.float16):
              with torch.no_grad():
                  embed = model.generate(prefix,kg,tokens,mask,q_len).view(1,tokens.size(0),-1)
                  if print_vis_token_meaning:
                    prefix_projections = embed[:,q_len+model.kg_len:q_len+model.kg_len+model.prefix_len,:]
                    for i in range(prefix_projections.size(1)):
                      print_nearest_text_token(prefix_projections[0,i], model)
                  out_text = generate_beam(model, model.tokenizer,generated=embed,entry_length=dataset.max_a_len, temperature=1)[0]
...
def print_nearest_text_token(vis_token, model):
    embeddings = model.model.transformer.wte.weight
...
if __name__ == '__main__':
    ...
    eval_gpt_open_ended(model, dataset, args)